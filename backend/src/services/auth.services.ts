...
export const userRegisterService = async (data: UserRegisterInput) => {
  ...
  let otpParseData = otpData ? JSON.parse(otpData) : {};
  if(!otpParseData || otpParseData == null){
   throw new ApiError("Otp is expired",401,false)
  }
  if(otpParseData.attempts > 3){
   throw new ApiError("You hit max attempt",401,false)
  }
  otpParseData.attempts = (otpParseData.attempts || 0) + 1;

  await redis.set(otpDataKey, JSON.stringify(otpParseData), {
    EX: 120, // 2 minutes
  });
